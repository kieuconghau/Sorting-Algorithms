...
/*
* GENERAL IDEA:
	- For simplicity, suppose that all elements in the array is non-negative
	and the position of each element is counted from 1 (instead of 0).
		a =		  { 9, 5, 2, 9, 1, 9, 2 }
		position    1  2  3  4  5  6  7

	- STEP 1: We need to know the FREQUENCY f(x) of each value in that array.
	...
	- STEP 2: Obviously, one value can appears in that array more than 1 time.
	...
	of the LAST element with value x in the final sorted array.
		x -> p(x)
		p(1) = f(1) = 1						-> the position of the last element with value 1 in the final sorted array
	...
	- STEP 3: We will sort this array base on the value of p(x) with x is 1,2,5,9.
		position    1  2  3  4  5  6  7
		p(x)		.     .  .        .
		a =		  { 1, 2, 2, 5, 9, 9, 9 }
*/
...
void CountingSort(vector<int>& a) {
	...
	// Step 1: Caculate FREQUENCY f(x) with x is each of all distinct values in that array.
	vector<int> fr(max - min + 1, 0);
	for (int i = 0; i < a.size(); ++i) {
		++fr[a[i] - min];		// a[i] - min: the value is started from 0 to max - min.
	}

	// Step 2: Caculate the position p(x) of the LAST element with x is each of all distinct values in that array.
	for (int i = 1; i < fr.size(); ++i) {
		fr[i] += fr[i - 1];
	}

	// Step 3: Put all elements of the array a to the array b in ascendingly sorted order.
